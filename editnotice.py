...
def add():
    if IdEntry.get() == "" or TopicEntry.get() == "" or DescriptionEntry.get(1.0, "end-1c") == "":
        ...
        mycursor = db.cursor()
        try:
            query = "INSERT INTO notice_board (id, topic, description) VALUES (%s, %s, %s)"
            values = (iD, name.upper(), desg)
...
def update():
    ...
    mycursor = db.cursor()
    try:
        query = "UPDATE notice_board SET topic = %s, description = %s WHERE id = %s"
        values = (name.upper(), desg, iD)
        mycursor.execute(query, values)
        db.commit()
...
#heading label
nsec=Label(header, text="IOE PULCHOWK CAMPUS",font=("Helvetica", 28,"bold"), bg = "brown",fg="#eae2b7")
